    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const driver = await axios.get('http://localhost:8099/api/user?role=DRIVER');
                        ...
                        <tr className="grid grid-cols-8" key={driverIndex}>
                            ...
                            {nextSevenDays.map((day, dayIndex) => {
                                const driverWorkday = workday.find(
                                    (item) => item.containerTruck.driver?.userId === driver.userId && (item.to >= day && item.from <= day)

                                );
                                console.log(day);
                                return <td className="ease relative h-20 cursor-pointer border border-stroke p-2 transition duration-500 hover:bg-gray dark:border-strokedark dark:hover:bg-meta-4 md:h-25 md:p-6 xl:h-31" key={dayIndex}>
                                    <span className="font-medium text-black dark:text-white">
                                        {driverWorkday ? <div className="event invisible absolute left-0 z-99 mb-1 flex w-[100%] flex-col rounded-sm border-l-[3px] border-primary bg-gray px-3 py-1 text-left opacity-0 group-hover:visible group-hover:opacity-100 dark:bg-meta-4 md:visible md:w-[100%] md:opacity-100">
